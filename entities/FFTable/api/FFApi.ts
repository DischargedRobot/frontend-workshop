...
const FFApi = {
    ...
    getFFsByDepartments: async (departmentIds: number[], organisationId: number, count: number, offset: number): Promise<{FFs: IFeatureFlag[], isEnd: boolean}> => {
        ...
        // const response = await Promise.all(departmentIds.map((departmentId) => FFApi.getFeatureFlagsByDepartment(departmentId, organisationId, 50, 0)))
    ...
    getFFsByDepAndItsChildren: async (departmentId: number, organisationId: number,): Promise<IFeatureFlag[]> => {
        const responseData = await APIJsonRequest<IFFByDepartmentLinkedResponse>(
    ...
    getFFsByDepsAndTheyChildren: async (departmentIds: number[], organisationId: number,): Promise<IFeatureFlag[]> => {
        const response = await Promise.all(departmentIds.map((departmentId) => FFApi.getFFsByDepAndItsChildren(departmentId, organisationId)))
